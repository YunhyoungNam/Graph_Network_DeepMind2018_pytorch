# -*- coding: utf-8 -*-
"""
Created on Fri Aug 19 12:32:28 2022
...
class InteractionNetwork(nn.Module):
    ...
    def __init__(self,
                 edge_model_info : dict,
                 node_model_info : dict,               
                 graph : GraphsTuple,
                 reducer='sum'):
      
        super(InteractionNetwork, self).__init__()      
        self._edge_block = blocks.EdgeBlock(
            edge_model_info, graph, use_globals=False)
...
class GraphNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
          edge_model_fn: A callable that will be passed to EdgeBlock to perform
            per-edge computations. The callable must return a MLP module (or
            equivalent; see EdgeBlock for details).
          node_model_fn: A callable that will be passed to NodeBlock to perform
            per-node computations. The callable must return a MLP module (or
            equivalent; see NodeBlock for details).
          global_model_fn: A callable that will be passed to GlobalBlock to perform
            per-global computations. The callable must return a MLP module (or
            equivalent; see GlobalBlock for details).
        ...
        self._edge_block = blocks.EdgeBlock(
            edge_model_info, graph, **edge_block_opt)
        self._node_block = blocks.NodeBlock(
            node_model_info, graph, **node_block_opt)
        self._global_block = blocks.GlobalBlock(
            global_model_info, graph, **global_block_opt)        
...
class GraphIndependent(nn.Module):
    ...
    def __init__(self,
               graph : GraphsTuple,
               edge_model_info=None,
               node_model_info=None,
        ...
        if edge_model_info is None:
            self._edge_model = lambda x: x
        else:
            self._edge_model = blocks.EdgeBlock(edge_model_info, graph)
        if node_model_info is None:
            self._node_model = lambda x: x
        else:
            self._node_model = blocks.NodeBlock(node_model_info, graph)
        if global_model_info is None:
            self._global_model = lambda x: x
        else:
            self._global_model = blocks.GlobalBlock(global_model_info, graph)
    ...
    def forward(self,
        ...
        return graph.replace(
            edges=self._edge_model(graph, **edge_model_kwargs).edges,
            nodes=self._node_model(graph, **node_model_kwargs).nodes,
            globals=self._global_model(graph, **global_model_kwargs).globals)  
...
def _unsorted_segment_softmax(data,
    ...
    """    
    segment_maxes = utils_pt.unsorted_segment_cal(data, segment_ids, num_segments, 'max')
    maxes = segment_maxes.index_select(index=segment_ids.long(), dim=0)
    # Possibly refactor to `tf.stop_gradient(maxes)` for better performance.
    data -= maxes    
    exp_data = torch.exp(data)
    
    segment_sum_exp_data = utils_pt.unsorted_segment_cal(exp_data, segment_ids, num_segments, 'sum')
    
    
    sum_exp_data = segment_sum_exp_data.index_select(index=segment_ids.long(), dim=0)    
...
def _received_edges_normalizer(graph,
    ...
    return normalizer(
        ...
        num_segments=torch.sum(graph.n_node,dtype=torch.int32))
...
class SelfAttention(nn.Module):
    ...
    def _build(self, node_values, node_keys, node_queries, attention_graph):
        ...
        attention_weights_logits = torch.sum(
